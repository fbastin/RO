...
& x_1, x_2, x_3 \geq 0,
\end{align*}
ce qui peut se traduire en Julia avec JuMP comme suit :
\begin{verbatim}
using JuMP
using HiGHS
...
println("Profit = ", objective_value(farm_model))
for c in crops
    println("Production($c) = ", value(production[c]))
end
for r in resources
    println("ResourceConstraint($r) = ", value(resource_con[r]))
end
\end{verbatim}
L'exécution de ce script Julia donne le résultat suivant :
\begin{verbatim}
Profit = 9950.0
Production(Corn) = 50.0
Production(Wheat) = 50.0
Production(Cotton) = 0.0
ResourceConstraint(Land) = 100.0
ResourceConstraint(Labor) = 500.0
\end{verbatim}

L'évaluation des contraintes indique que les deux contraintes, sur la terre (Land) et la main d'œuvre (Labor), sont actives (leur niveau atteint la valeur maximale de 100 et 500 respectivement) : toutes les ressources sont employées.
Les variables duales optimales correspondantes peuvent par conséquent être non nulles et représentent la valeur marginale (shadow price) de ces contraintes.
Nous voyons en particulier que la terre a la plus grande valeur marginale.

Les quantités de cultures produites montrent que seuls le maïs (Corn) et le blé (Wheat) sont produits, mais pas le coton (Cotton).

En JuMP, nous pouvons obtenir les valeurs des variables duales (les multiplicateurs de Lagrange / variables duales associés aux contraintes) et les coûts réduits des variables à l'aide des fonctions \verb|shadow_price| et \verb|reduced_cost| :
\begin{verbatim}
# Obtenir les prix d'ombre (shadow prices) des contraintes
for r in resources
    println("Shadow price ($r) = ", shadow_price(resource_con[r]))
end

# Obtenir les coûts réduits (reduced costs) des variables de décision
for c in crops
    println("Reduced cost ($c) = ", reduced_cost(production[c]))
end
\end{verbatim}
Ce qui donne en sortie :
\begin{verbatim}
Shadow price (Land) = 52.0
Shadow price (Labor) = 9.5
Reduced cost (Corn) = -0.0
Reduced cost (Wheat) = -0.0
Reduced cost (Cotton) = -13.0
\end{verbatim}
La fonction \verb|value| renvoie la valeur optimale d'une variable ou l'expression d'une contrainte à l'optimum.
La fonction \verb|shadow_price| (ou \verb|dual|) permet d'obtenir le multiplicateur optimal associé à une contrainte.
Dans le cas d'une variable de décision, la fonction \verb|reduced_cost| permet de connaître son coût réduit. Ici, le coût réduit du coton est de -13,0, ce qui signifie que le profit de l'agriculteur diminuerait de 13,0 par acre de coton s'il était contraint d'en cultiver une quantité infinitésimale.
\end{example}
